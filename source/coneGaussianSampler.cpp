...
ConeGaussianSampler::ConeGaussianSampler() {
  ...
  max_gaussian_packing = CONEPACKING::_3;

  covered_distance = 221.7025;
...
void ConeGaussianSampler::ComputeConeIntegrationSteps(double min_sg_gaussian) {
  ...
  double curr_pos = initial_step; // 即 h_0 = 3.0 * sigma_0
  double sigma_gaussian = min_sg_gaussian; // sigma_0 = 1.0
  ...
  while (curr_pos < GetCoveredDistance())
  {
    ...
    double si = (GetIntegrationHalfStepMultiplier() * sigma_gaussian); // d_sigma = 1.25
...
bool ConeGaussianSampler::AddGaussianSampleStep(double curr_pos, double sg_gaussian, int* n_gaussians)
{
  ...
  if (cone_radius > GetGaussianSigmaLimitMultiplier() * sg_gaussian) // GetGaussianSigmaLimitMultiplier = r_sigma = 2.0;
  {
